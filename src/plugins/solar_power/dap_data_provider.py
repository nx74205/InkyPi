...
class DapDataProvider:
    """
    Provider for Day Ahead Price (DAP) data from Energy-Charts API.
    Handles fetching and formatting of electricity price data.
    """
    
    def __init__(self):
        """Initialize the DAP data provider."""
        self.api_base_url = "https://api.energy-charts.info/price"
        self.api_timeout = 10
    ...
    def _fetch_renewable_share_forecast(self, country="de"):
        ...
        try:
            ...
            response = requests.get(base_url, params=params, timeout=self.api_timeout)
            response.raise_for_status()
