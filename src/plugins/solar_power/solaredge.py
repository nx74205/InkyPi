...
class Solaredge(SolarProvider):
    ...
    def get_chart_data(self):
        ...
        grid_charged_hourly = storage_data['gridCharged']
        ...
        if energy_data and 'energy' in energy_data:
            ...
            for i, value_dict in enumerate(values):
                if i < 24:
                    ...
                    gridCharged =  grid_charged_hourly[i] if i < len(grid_charged_hourly) else 0
                    discharged = discharged_hourly[i] if i < len(discharged_hourly) else 0
                    
                    # Calculate: value + charged - discharged (with minimum of 0)
                    hourly_data[i] = max(0, value + (charged - gridCharged) - discharged)
    ...
    # Private data fetching methods - Energy
    ...
    # Private data fetching methods - Storage/Battery
    ...
    def _fetch_hourly_storage_data(self, date):
                ...
                for telemetry in telemetries:
                    ...
                    if time_stamp:
                        try:
                            ...
                            grid_charged = telemetry.get('ACGridCharging')
                            ...
                            if lifetime_charged is not None or lifetime_discharged is not None:
                                hourly_telemetries[hour].append({
                                    ...
                                    'gritCharged': grid_charged
                                })
                            
                            # Sum grid charged values directly
                            if grid_charged is not None and grid_charged > 0:
                                grid_charged_hourly[hour] += grid_charged
        ...
        return {
            ...
            'gridCharged': grid_charged_hourly
        }
    
    # Private data fetching methods - Power
